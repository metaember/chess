...
pub fn search_with_book(
    ...
    if let Some(mv) = book.suggest_move(board, true) {
        return SearchResult {
            best_move: Some(mv),
            best_score: 0,
            nodes_searched: 0,
    ...
    iterative_deepening_movepicker(&mut board, max_depth, &mut tt)
}

pub fn search(max_depth: u8, board: &Board) -> Move {
    let mut board = board.clone();
    let mut tt = TranspositionTable::new(16);
    let result = iterative_deepening_movepicker(&mut board, max_depth, &mut tt);
...
mod test {
    ...
    fn test_quiescence_search_1() {
        // Black pawn on e3 can capture white rook on f2.
        // With quiescence, engine sees the rook capture is good.
        let mut b = Board::from_fen("6R1/8/8/8/8/4p3/5R2/5K1k b - - 0 1");
        let mut tt = TranspositionTable::new(1);
        let result = iterative_deepening_movepicker(&mut b, 1, &mut tt);
    ...
    fn test_quiescence_search_2() {
        // White rook on e4 can capture pawn on e5, but black recaptures.
        // With quiescence, engine should not play the rook capture.
        let mut b = Board::from_fen("8/5k2/5p2/4p3/4R3/8/8/3K4 w - - 0 1");
        let mut tt = TranspositionTable::new(1);
        let result = iterative_deepening_movepicker(&mut b, 1, &mut tt);
        let m = result.best_move.unwrap();
    ...
    fn test_quiescence_search_3() {
        // White can capture pawn, black recaptures rook, but white recaptures with queen.
        // Net result is favorable for white.
        let mut b = Board::from_fen("4k3/8/4r3/8/4p3/3p4/8/3RQK2 w - - 0 1");
        let mut tt = TranspositionTable::new(1);
        let result = iterative_deepening_movepicker(&mut b, 1, &mut tt);
        let m = result.best_move.unwrap();
        // Engine should find a capture (not a non-capture)
        assert!(m.captured.is_some(), "Engine should capture: got {}", m.to_algebraic());
    }

    #[test]
    fn test_quiescence_search_4() {
        // White rook captures pawn but loses it — should avoid.
        let mut b = Board::from_fen("4k3/8/4r3/8/4p3/3p4/8/3R1K2 w - - 0 1");
        let mut tt = TranspositionTable::new(1);
        let result = iterative_deepening_movepicker(&mut b, 1, &mut tt);
        let m = result.best_move.unwrap();
    ...
    fn test_does_not_blunder_knight() {
        ...
        assert_ne!(best_move, blunder, "Engine should not blunder f3");
        assert_eq!(
            best_move.piece.piece_type,
            PieceType::Knight,
            "Engine should move the knight. Got: {} {}",
            best_move.to_algebraic(), best_move.to_human()
        );
    }
